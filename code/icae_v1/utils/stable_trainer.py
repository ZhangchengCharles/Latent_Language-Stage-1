...
class StableTrainer:
    ...
    def create_optimizer(self):
        ...
        if self.optimizer is None:
            ...
            optimizer_cls, optimizer_kwargs = StableTrainer.get_optimizer_cls_and_kwargs(self.args)
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                with self.accelerator.accumulate(model):
                    _tr_loss_step = self.training_step(model, inputs)   # loss, tokens, denominator
                    loss_logs.append(_tr_loss_step)

                    # for debug only
                    # self.accelerator.unscale_gradients(optimizer=self.optimizer)
                    # grad_norm = compute_grad_norm(self.optimizer)
                    # print(f"grad_norm: {grad_norm}; rank: {torch.distributed.get_rank()}; local_step: {total_batched_samples}")

                    if total_batched_samples % args.gradient_accumulation_steps == 0:
                        accumulated_loss, accumulated_tokens, accumulated_denominator = sum(e['loss'] for e in loss_logs), sum(e['num_tokens'] for e in loss_logs), sum(e['loss_denominator'] for e in loss_logs)
                        loss_logs = []  # clear los for the next global step.

                        # collect loss/tokens for reduce
                        accumulated_loss_tokens_denominator = torch.tensor([accumulated_loss, accumulated_tokens, accumulated_denominator], device=self.accelerator.device)

                        # reduce -> sum
                        if torch.distributed.is_initialized():
                            dist.all_reduce(accumulated_loss_tokens_denominator, op=dist.ReduceOp.SUM)

                        # get reduced value
                        reduced_avg_loss = accumulated_loss_tokens_denominator[0] / dist.get_world_size()   # average_loss
                        reduced_tokens = accumulated_loss_tokens_denominator[1].item()
                        reduced_denominator = accumulated_loss_tokens_denominator[2].item()

                        # get global-step loss
                        tr_loss_step = reduced_avg_loss * reduced_denominator / reduced_tokens  # micro-average loss, like fairseq
                        if (
                        ...
                        def multiply_gradient(optimizer, c):
                            per_device_and_dtype_grads = defaultdict(lambda: defaultdict(list))
                            for group in optimizer.param_groups:
                                for param in group["params"]:
                                    if param.grad is not None:
                                        if param.grad.is_sparse:
                                            param.grad.data.mul_(c.to(param.grad.device) if torch.is_tensor(c) else c)
                                        else:
                                            per_device_and_dtype_grads[param.grad.device][param.grad.dtype].append(
                                                param.grad.data
                                            )
                        ...
                        multiply_gradient(self.optimizer, reduced_denominator / reduced_tokens)

                        # for debug only
                        # self.accelerator.unscale_gradients(optimizer=self.optimizer)
                        # grad_norm = compute_grad_norm(self.optimizer)
                        # print(f"grad_norm: {grad_norm}; rank: {torch.distributed.get_rank()}; global_step: {self.state.global_step}")
                ...
                if (
                    ...
                    # after_step_grad = compute_grad_norm(self.optimizer)
                    # print(f"after_step_grad: {after_step_grad}; rank: {torch.distributed.get_rank()}; step: {self.state.global_step}")
    ...
    def training_step(self, model: nn.Module, inputs: Dict[str, Union[torch.Tensor, Any]]) -> torch.Tensor:
        ...
        with self.compute_loss_context_manager():
            loss, num_tokens, loss_denominator, original_loss = self.compute_loss(model, inputs, token_avg_loss=True)
        ...
        log_loss = loss.detach() if original_loss is None else original_loss

        return {'loss': log_loss / self.args.gradient_accumulation_steps, 'num_tokens': num_tokens, 'loss_denominator': loss_denominator}

    def compute_loss(self, model, inputs, return_outputs=False, token_avg_loss=False):
        """
        ...
        if self.label_smoother is not None and "labels" in inputs:
            ...
            labels = None
        outputs = model(**inputs, token_avg_loss=token_avg_loss)

        loss = outputs['loss']
        logits = outputs['logits']
        original_loss = None if 'original_loss' not in outputs else outputs['original_loss']    # original_loss is not None only when training VQ-ICAE

        if token_avg_loss:
            loss_denominator = outputs['loss_denominator']
            num_tokens = inputs['labels'].ne(-100).sum()
            return (loss, num_tokens, loss_denominator, original_loss) if return_outputs is False else (loss, num_tokens, loss_denominator, logits, original_loss)
        else:
            return (loss, original_loss) if return_outputs is False else (loss, logits, original_loss)
    ...
    def prediction_step(
        ...
        with torch.no_grad():
            if is_sagemaker_mp_enabled():
                ...
                if has_labels or loss_without_labels:
                    with self.compute_loss_context_manager():
                        loss, outputs, original_loss = self.compute_loss(model, inputs, return_outputs=True, token_avg_loss=False)
